...
public:
    ...
    {
        // 인자로 new(동적할당) 객체 형식으로 올 수 있음
        // _data에 동적할당 받은 객체의 주소가 들어가는 형식
        Node* newNode = new Node;
        newNode->_data = data;
    ...
    void pop_front()
    {
        ...
        delete deleteNode;
        // 이때 노드의 동적할당만 해제된거고 
        // 만약 노드 내부의 _data의 값도 동적할당된 주소라면 개별적으로 delete를 해줘야 함
    ...
    // _data 속 동적할당된 부분은 delete되지 않기 떄문에 개별적으로 해제해야하는 부분
    // 리스트의 노드만이 clear
    void clear()
    {
        iterator iter;
